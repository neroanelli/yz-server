var myDataTable;
		$(function() {
			...
			myDataTable = $('#textBookTable').dataTable(
					{
						"serverSide" : true,
						"dom" : 'rtilp',
						"ajax" : {
							...
							data : {
								"goodsName" : function(){
									return $("#goodsNameT").val();
								},
								"isAllow" : function(){
									return $("#isAllowTextBook").val();
								},
								"goodsType" : function(){
									return "4";
								}
						...
						"columnDefs" : [
									...
									"render" : function(data, type, row, meta) {
										var dom = '';
										dom += '<a title="编辑" href="javascript:void(0)" onclick="textbook_edit(\'' + row.goodsId + '\')" class="ml-5" style="text-decoration:none">';
		...
		/*管理员-普通商品-添加*/
		function textbookAdd() {
			var url = '/goods/toTextBookEdit.do' + '?exType=Add';
			layer_show('添加普通商品', url, null, 510, function() {
				myDataTable.fnDraw(true);
			},true);
		}
		/*管理员-普通商品-编辑*/
		function textbook_edit(goodsId) {
			var url = '/goods/toTextBookEdit.do' + '?goodsId='+ goodsId +'&exType=UPDATE';
