	...
	$("#form-auction-info").validate({
		...
		submitHandler : function(form) {
			...
			$(form).ajaxSubmit({
				type : "post", //提交方式  
				dataType : "json", //数据类型  
				url : "/auction/auctionUpdate.do", //请求url  
...
function deletPhoto() {
      ...
      function showAuctionResult(){
      	...
      	var url = '/auction/showAuctionResult.do' + '?salesId='+ salesId;
      	layer_show('查看往期中拍记录', url, null, 510, function() {
      		myDataTable.fnDraw(true);
      	});
      }
  	
  	 function validateGoodsCount(){
  		var goodsCount = $("#totalCount").val(); 
