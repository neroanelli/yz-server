var myDataTable;
		$(function() {
			...
			myDataTable = $('#activityTable').dataTable(
					{
						"serverSide" : true,
						"dom" : 'rtilp',
						"ajax" : {
							...
							data : {
								"goodsName" : function(){
									return $("#goodsNameA").val();
								},
								"isAllow" : function(){
									return $("#isAllowActivity").val();
								},
								"goodsType" : function(){
									return "3";
								}
						...
						"columnDefs" : [
									...
									"render" : function(data, type, row, meta) {
										var dom = '';
										dom += '<a title="编辑" href="javascript:void(0)" onclick="activity_edit(\'' + row.goodsId + '\')" class="ml-5" style="text-decoration:none">';
		...
		function activityAdd() {
			var url = '/goods/toActivityEdit.do' + '?exType=Add';
			layer_show('添加活动商品', url, null, 510, function() {
				myDataTable.fnDraw(true);
			},true);
		}
		/*管理员-线下活动-编辑*/
		function activity_edit(goodsId) {
			var url = '/goods/toActivityEdit.do' + '?goodsId='+ goodsId +'&exType=UPDATE';
			layer_show('修改活动商品', url, null, 510, function() {
