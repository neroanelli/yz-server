...
$(function(){
	_init_checkbox("goodsUse","goodsUse",
	  ...
	  _init_select("courseType", dictJson['courseType'],varGoodsInfo.courseType);
	  initMap();//初始化地图
...
function initMap() {
    ...
    geolocation.getCurrentPosition(function (r) {
        if (this.getStatus() == BMAP_STATUS_SUCCESS) {
           map = BaiduMap("addressMap", "address", true, r.point.lng, r.point.lat);
        }
        else {
           map = BaiduMap("addressMap", null, true);
        }
    }, {enableHighAccuracy: true});
    $("#address").bind('input propertychange', function () {
        var address = $(this).val();
        searchLocal(map, address);
    })
}
function deletPhoto() {
