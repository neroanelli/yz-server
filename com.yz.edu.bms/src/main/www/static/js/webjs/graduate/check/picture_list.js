  var myDataTable;
        $(function () {
            ...
            $("#unvsIdPicture").append(new Option("", "", false, true));

            //初始化年级下拉框
            _init_select("gradePicture", dictJson.grade);

            //初始化院校类型下拉框
            _init_select("recruitTypePicture", dictJson.recruitType);

            myDataTable = $('#pictureListTable').dataTable(
                {
                    "serverSide": true,
                    "dom": 'rtilp',
                    "ajax": {
                        url: "/check/pictureList.do",
                        data: {
                            "gCheckType": "2",
                            "unvsId": function () {
                                return $("#unvsIdPicture").val();
                            }, "recruitType": function () {
                                return $("#recruitTypePicture").val();
                            }, "grade": function () {
                                return $("#gradePicture").val();
                            }, "pfsnName": function () {
                                return $("#pfsnName").val();
                            }, "stdName": function () {
                                return $("#stdName").val();
        ...
        function check_data(checkId, learnId, stdName,exType) {
            var url = '/check/checkPicture.do' + '?checkId=' + checkId + '&learnId=' + learnId + '&stdName=' + stdName + '&exType='+exType;
            layer_show('毕业图像核查', url, 1000, 510, function () {
