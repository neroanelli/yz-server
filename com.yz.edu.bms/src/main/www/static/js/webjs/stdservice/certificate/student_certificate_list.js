var myDataTable;
    $(function () {
        ...
        _init_select("applyType", [
            {"dictValue": "1", "dictName": "在读证明"},
            {"dictValue": "2", "dictName": "课时证明"},
            {"dictValue": "3", "dictName": "发票"},
            {"dictValue": "4", "dictName": "收据"},
        ...
        _init_select("teacherRemark", [
            {"dictValue": "2", "dictName": "无备注"},
            {"dictValue": "1", "dictName": "有备注"}
        ]);
        
        _init_select("isGiveOut", [
           {"dictValue": "0", "dictName": "未发放"},
           {"dictValue": "1", "dictName": "已发放"}
       ]);
        ...
        myDataTable = $('.table-sort').dataTable({
            ...
            "dom": 'rtilp',
            "ajax": {
                url: '/certificate/findAllList.do',
                type: "post",
            ...
            "columnDefs": [
                ...
                },
                {
                    ...
                    if (row.applyType && row.applyType == '1') {
                        return "在读证明";
                    }
                    else if (row.applyType && row.applyType == '2') {
                        return "课时证明";
                    }
                    else if (row.applyType && row.applyType == '3') {
                        return "发票";
                    }
                    else if (row.applyType && row.applyType == '4') {
                        return "收据";
                    } else {
                        return "<p class='overflowEllipsis' title='"+row.materialName+"'>其它 "+"<span style='color:red'>（"+row.materialName+"）<span></p>";
                    }
                }
                },
                {
                ...
                },
                {
                    "targets": 14, "render": function (data, type, row, meta) {
                    	var teacherRemark = row.teacherRemark ? row.teacherRemark : "", certId = row.certId ?　row.certId : "", dom='';
                        if(!!teacherRemark){
                            dom='<a href="javascript:void(0);" class="remarkBox" style="text-decoration: none">'+teacherRemark+'<i title="修改备注" onclick="showEditRemark(\''+row.certId+'\')" class="Hui-iconfont Hui-iconfont-shuru remarkIcon"></i></a>';
                        }else {
                            dom = '<a title="添加备注" href="javascript:void(0);" onclick="showEditRemark(\''+row.certId+'\')" style="text-decoration: none">';
                ...
                },
                {
                    ...
                    var dom = '';
                    if(row.applyType=='1' && row.checkOrder=='2' && row.checkStatus == '2'){
                    ...
                    if (row.checkOrder == '2' && row.checkStatus == '2') {
                        dom += '<a title="发货" href="javascript:;" onclick="addExpressNo(\'' + row.certId + '\', \'' + row.isGiveOut + '\', \'' + row.expressNo + '\')" class="ml-5" style="text-decoration: none">';
    ...
    function showEditRemark(certId) {
        var url = '/certificate/toEditRemark.do' + '?certId=' + certId;
    ...
    /*查看详情*/
    function view(id) {
        var url = '/certificate/view.do' + '?certId=' + id;
    ...
    function print(learnId) {
        ...
        var printUrl = '/certificate/printView.do'+ '?learnId=' + learnId;
    ...
    /*发货*/
    function addExpressNo(certId,isGiveOut,expressNo) {
    	if(expressNo!="null"){
    		$("#addExpressNo").val(expressNo);
    	}else{
    		$("#addExpressNo").val("");
    	}
    	if(isGiveOut!="null"){
    		$("input[name='isGiveOut'][value='"+isGiveOut+"']").prop("checked",true); 
    	}else{
    		$("input[name='isGiveOut'][value='0']").prop("checked",true); 
    	}
        var url = '/certificate/deliver.do';
        layer.open({
            type: 1,
            btn: ['确定'],
            title: '发货设置',
            yes: function (index, layero) {
                ...
                var isGiveOut = $("input[name='isGiveOut']:checked").val();
                /*if (addExpressNo == '') {
                    layer.msg("请填写快递单号");
                    return;
                }*/
                if (addExpressNo.length > 20) {
                    layer.msg('快递单号最大支持20个字符!', {
                        icon: 2,
                        time: 2000
                    });
                    return;
    ...
    function certificateImport() {
        var url = '/certificate/certificateImport.do';
        layer_show('学员证明申请导入', url, null, 510, function () {
