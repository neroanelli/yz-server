function initItemDom() {
        ...
        for (var i = 0; i < row.length; i++) {
            ...
            dom += '<td>' + '<input type="hidden" value="' + payInfo.itemCode + '" name="items[' + i + '].itemCode" id="itemCode" />' + '<input type="number" class="input-text radius" value="' + payInfo.refundAmount + '" name="items[' + i + '].refundAmount" id="refundAmount" max="' + payInfo.payable + '"/>' + '</td>';
            dom += '</tr>';
        }
        dom += '</tbody>';
        $("#feeItem").append(dom);
            ...
            submitHandler: function (form) {
                ...
                if ('2' == $('#checkStatus').val()) {
                    ...
                    $(form).ajaxSubmit({
                        type: "post", //提交方式
                        dataType: "json", //数据类型
                        url: '/studentOutApproval/passSchoolManagedApproval.do', //请求url
