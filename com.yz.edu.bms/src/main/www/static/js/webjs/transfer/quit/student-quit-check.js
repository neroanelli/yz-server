	 ...
	 //初始化院校名称下拉框
	   ...
	   $("#pfsnId").append(new Option("", "", false, true));
		...
		myDataTable = $('.table-sort').dataTable({
			...
			"dom" : 'rtilp',
			"ajax" : {
				url : "/quitSchoolCheck/findAllBdStudentQuit.do",
				type : "post",
				data : {
					"stdName" : function() {
						return $("#stdName").val();
			...
			"columnDefs" : [ {
				"render" : function(data, type, row, meta) {
					if(row.checkStatus=='2'){
						return '<input type="checkbox" value="'+ row.id+';'+row.learnId + '" name="ids"/>';	
					}else{
						return '';
					}
				},
			...
			,{
				"render" : function(data, type, row, meta) {
					var dom = '';
					if(row.checkStatus =='2'){
						dom = '<a title="审批" href="javascript:;" onclick="checkQuit(\''+ row.id+ '\')" class="ml-5" style="text-decoration: none">审批</a>';
					}else{
						dom = '<a title="查看" href="javascript:;" onclick="lookQuit(\''+ row.id+ '\')" class="ml-5" style="text-decoration: none">查看</a>';
					}
					return dom;
				},
				//指定是第10列是操作列
...
function checkQuit(id) {
	var url = '/quitSchoolCheck/lookQuitSchool.do' + '?id=' + id;
	layer_show('审核休学学员信息', url, null, 700, function() {
	});
}
...
function batchOper() {
	...
	var url = '/quitSchoolCheck/toSelectChecks.do';
	layer_show('批量审核休学申请', url, 500, 275, function() {
		myDataTable.fnDraw(false);
	});
}
function exportStudentQuit(){
	$("#export-form").submit();
}