...
$(function() {
	...
	myDataTable = $('.table-sort').dataTable({
		...
		"dom" : 'rtilp',
		"ajax" : {
			url : "/allStudent/findAllStudent.do",
			type : "post",
		...
		"columnDefs" : [
            ...
            {"targets" : 7,"render" : function(data, type, row, meta) {
            	var dom ='';
            	dom = row.recruitName == null ? '智哥':row.recruitName;
            	if(row.empStatus ==='2'){
            		dom += ' <span class="name-mark-out">离</span>';
            	}
                return dom; 
            },"class":"no-warp"},
            { "targets" : 9,"class":"text-c","render" : function(data, type, row, meta) {
                    var remark = row.remark ? row.remark : "", userId = row.userId ?　row.userId : "";
                    if(!!remark){
                        var dom='<a href="javascript:void(0);" class="remarkBox" style="text-decoration: none">'+remark+'<i title="修改备注" onclick="showEditRemark(\'' + userId + '\', \''+row.stdId+'\')" class="Hui-iconfont Hui-iconfont-shuru remarkIcon"></i></a>';
                    }else {
                        var dom = '<a title="添加备注" href="javascript:void(0);" onclick="showEditRemark(\'' + userId + '\', \''+row.stdId+'\')" style="text-decoration: none">';
                        dom += '<i class="iconfont icon-shenhe"></i></a>';
                    }
                    return dom;
            }},
            {"targets" : 10,"render" : function(data, type, row, meta) {
				var dom = '<a title="编辑" href="javascript:void(0);" onclick="toEidt(\'' + row.learnId + '\',\'' + row.stdId + '\',\'' + row.recruitType + '\')" style="text-decoration: none">';
...
function unvsText(data) {
	var pfsnName = data.pfsnName, unvsName = data.unvsName,recruitType = data.recruitType,
		pfsnCode = data.pfsnCode, pfsnLevel = data.pfsnLevel;
