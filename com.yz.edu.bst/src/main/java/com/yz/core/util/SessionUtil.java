...
import com.yz.core.constants.SecurityConstant;
...
public class SessionUtil {
	...
	public static LoginUser getUser() {
		return RedisService.getRedisService().getByte(RequestUtil.getSessionId(), LoginUser.class);
	}
	...
	public static void setUser(LoginUser user) {
		RedisService.getRedisService().setex(RequestUtil.getSessionId(), FstSerializer.getInstance().serialize(user), SESSION_TIMEOUT);
	}
	...
	public static void pushToken(String groupId, String token) {
		//MapCacheUtil map = RemoteCacheUtilFactory.map(RequestUtil.getSessionId()).setLiveTime(24 * 3600);
	...
	public static String pullToken(String groupId) {
		//MapCacheUtil map = RemoteCacheUtilFactory.map(RequestUtil.getSessionId());
		//String token = (String) map.get(groupId);
		//map.remove(groupId);
		//return token;
		return null;
	}
	...
	public static void setKey(String userId) {
		//SimpleCacheUtil simple = RemoteCacheUtilFactory.simple().setLiveTime(SESSION_TIMEOUT);
		//simple.put(SecurityConstant.SESSION_KEY_PREFIX + userId, RequestUtil.getSessionId());
		RedisService.getRedisService().setex(SecurityConstant.SESSION_KEY_PREFIX + userId, RequestUtil.getSessionId(),SESSION_TIMEOUT);
	}
	...
	public static String getKey(String userId) {
		...
		//return (String) simple.get(SecurityConstant.SESSION_KEY_PREFIX + userId);
	...
	public static void freshTime(String sessionId, String userId) {
		/*SimpleCacheUtil simple = RemoteCacheUtilFactory.simple().liveTime(SESSION_TIMEOUT);
