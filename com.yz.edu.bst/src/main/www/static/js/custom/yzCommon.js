...
$(function() {
	...
	$.ajaxSetup({
	    beforeSend : function(){
	        /* layer.load(2, {time: 10*1000, shade : 0.3}); */
	    },
...
function _init_checkbox(cbBoxId, cbName, dictArray, checkedValue) {
    	...
    	for(var i=0; i<dictArray.length; i++) {
    		...
    		var dom = "<div class='check-box' style='width: 150px;padding-left:0'>";
 ...
 * @returns 邮编
 */
function _init_area_select(pSelectId, cSelectId, dSelectId, psv, csv, dsv) {
	$("#" + pSelectId).empty();
...
function _showOther(obj){
	...
	var moreHeight={};
			...
			if(moreHeight.height){
					...
					moreHeight.height=parseInt(search.find('.cl:first-child').css('height'))+45;
			}
		
			if(moreHeight.height<90){
	        layer.msg('没有更多的选项了哦', {
                icon : 0,
                time : 3000
            });
    ...
    $.fn.dataTableExt.oPagination.full_numbers= {
        ...
        'fnInit': function(oSettings, nPager, fnCallbackDraw) {
            ...
            inputSpanT.on('click',function () {
                ...
                var pageValue = parseInt($(inputIn).val(), 10) - 1 ; // -1 because pages are 0 indexed, but the UI is 1
                var oPaging = oSettings.oInstance.fnExtStylePagingInfo();
